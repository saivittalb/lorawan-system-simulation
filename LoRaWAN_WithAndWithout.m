 ...
 max_run=500;                            %Maximum number of iterations for a single SNR
 Eb=1;                                    %Bit energy
 ...
 BER_th1=(1/2)*erfc(sqrt(SNR/1.15));      %Calculate analytical BER with Golay
 BER_th2=(1/2)*erfc(sqrt(SNR/2));         %Calculate analytical BER without Golay
 close(hand);
 
 semilogy(SNRdB,BER_th2,'r-o','linewidth',1);    %Plot BER without Golay
 grid on, hold on;
 semilogy(SNRdB,BER_th1,'b-^','linewidth',1);    %Plot BER with Golay
 legend('Without Golay Codes','With Golay Codes');
 title('BER vs SNR for LoRaWAN with and without Golay (GFSK)');
 xlabel('SNR (dB)');
 ylabel('BER');
